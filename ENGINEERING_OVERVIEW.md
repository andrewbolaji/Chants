...
`functions/package.json`, its root lock metadata, and the Functions CI job target Node 22. PR 25's exact-head run `33340779709` passed all eight jobs at `d7b8b6f`. The current safety block adds no dependency version changes. It expands local Functions coverage to 202 unit tests plus 12 dedicated real-emulator transaction cases. The existing rules CI job now uses Node 22 to build and run that rehearsal; seed stays Node 20. This snapshot credits local safety evidence only; PR 25 records the resulting commit and replacement clean-runner outcome after authorized packaging.
...
`functions/src/report_repair.ts` provides a bounded plan/apply operator library. Its CLI pins project, reviewed source, private plan and digest. It scans parents including zero-report rows, refuses overflow, never unhides, and commits count, parent, audit and progress together before separate readback. `report_cutover.ts` validates pause attestations and exactly two replacement identities; it neither proves live containment nor deploys or reopens anything. Decision 026 and the runbook state the remaining operational limits.
...
The next combined Claude review starts at last-reviewed `cb50d3cc966c6a367309c887a8c765891155cf0e` and ends at the final safety head after separately authorized packaging and exact-head CI, covering PRs 22-25 and this block. Prior optional Living Songbook findings remain in the readiness record. First review operational admission, two-document upload authority, every grant writer, deferred cleanup, repair checkpoints and the distinction between synthetic cutover evidence and actual containment.
