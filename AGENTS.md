...
cd functions && npm ci && npm run build && npm test  # Node 22, 205 unit tests; 18 emulator-only cases skip here
...
firebase emulators:exec --only firestore --project demo-chants-repair "cd functions && npx mocha --timeout 60000 lib-test/test/report_repair.integration.test.js"  # 18 transaction cases, after Functions test compilation
