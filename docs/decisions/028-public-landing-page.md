...
- **Status:** Accepted visually; post-review correction is locally verified while replacement packaging, exact-head CI, final review closure, merge, deployment, DNS, and live readback remain separate gates
- **Date:** 2026-09-01
...
Continue the same launch vocabulary inside Flutter. The operating-system splash uses ink black. The first Flutter-owned frame reveals `CHANTS` once while Firebase and account gates resolve behind it, but the reveal never owns authority or delays reduced-motion users. If initialization outlives the one-shot reveal, the same surface exposes a written and semantic busy cue until the real gate takes ownership. Compact landscape and enlarged-text layouts adapt without clipping. The current-policy gate uses a loud football header followed by a calm constrained reading surface. It explicitly names Terms and Community Rules as the accepted documents, keeps Privacy separate, preserves Help, Support, Delete account, and Sign out before acceptance, and keeps a wrapping agreement action within the viewport.
...
- Two local font copies add static bytes but avoid third-party requests and preserve the app's existing visual system. Nunito carries display and reading copy, and Oswald handles compact signage. The unused hosted Anton copy is deliberately absent.
- Reusing the approved scarf mark avoids rights and provenance uncertainty from stock or generated football imagery.
- The two product devices make the app legible before launch without adding image rights, a new dependency, or another visual asset provenance boundary. Their club-neutral CSS composition can evolve with the product and deliberately avoids both generic stadium graphics and unsupported production screenshots. Each illustration exposes one useful accessible label while its simulated headings, tabs, and controls stay out of the accessibility tree. The visible five-tab shell follows the actual Feed, Clubs, Create, Songbook, and You navigation.
...
Deterministic Node tests cover root metadata, exact operator spelling, each product lane, trust meaning, launch honesty, all six trust routes, absence of collection, remote resources, fabricated audience metrics, seeded club names, chant or lyric-shaped illustration copy, stadium-proof conflation and store CTAs, illustration semantics, the five-tab shell, the Apple association source, and existing Firebase rewrites. Focused Flutter tests cover app-level reveal handoff, reveal timing, reduced motion, busy state, compact landscape, semantics, gate meaning, route access, pending and failure behavior, narrow width, agreement geometry, and enlarged text. The reviewed head passed the complete Flutter suite, CI-equivalent analysis, Functions, seed, source contracts, launch-service contracts, staged governance, writing-style controls, native-project contract, and exact-head clean-runner CI. The post-review correction passes 524 complete Flutter tests, clean `lib` and `test` analysis, all 37 focused Node contracts, and the launch-services contract plus its regression test locally. Local HTTP checks cover the root, all six trust routes, association source, stylesheet, fonts, and preview asset. Desktop, 390-pixel, and 320-pixel browser inspection cover the accepted public direction. Java-backed rules execution for the correction head, a final physical-device pass, targeted independent review closure, staged governance, and replacement clean-runner CI remain handoff gates.
