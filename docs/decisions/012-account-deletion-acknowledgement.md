...
Local deletion staging has three durable meanings: `prepared` before the network attempt, `unknown` before awaiting the remote response, and `accepted` after the callable explicitly confirms durable acceptance. Startup may restore prepared data and remove accepted data. A serialized explicit recovery may also restore `prepared` in the same process after a pre-network failure. It clears the successful initialization memo, reruns artifact recovery, and returns the resulting state. The app must neither restore nor discard `unknown` data.

If staging or the prepared-to-unknown transition fails, the remote callable is not invoked. The signed-in gate treats a surviving `prepared` marker as recoverable local state and actively runs recovery before deciding whether Home is safe. A recovery error remains fail-closed behind `RECOVERY NEEDED`, `TRY RECOVERY`, and Sign out; retry performs another recovery attempt rather than rereading an unchanged marker.
...
- Positive: a local failure before the network boundary can restore prepared data without requiring process relaunch.
...
- **Evidence:** Fresh and same-process repository tests cover prepared recovery, unknown preservation, accepted-last cleanup, conflicting artifacts, transient initialization retry, ambiguous response, retry acceptance, positive pending reconciliation, and sign-out behavior. A service test proves a pre-network transition failure restores staged data and never calls the remote boundary. App-gate tests and inspected 390 by 844 goldens verify persistent uncertainty and fail-closed recovery states.
