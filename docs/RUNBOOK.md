...
Read-only inventory on 2026-08-30 established that production still used the July backend, not the reviewed V1 Functions source. Gates 0 and 1 on 2026-09-01 confirmed that state, established recovery and containment, and closed production at maintenance generation 1. Gate 2 then replaced the predecessor set with exactly 46 non-scheduled Node 22 Functions from one reviewed source build while admission remained closed. Gate 3 and its three separately approved retries opened core generations 2, 4, 6, and 8 and returned to maintenance generations 3, 5, 7, and 9. Workers remained false throughout. No opening received an owner request, so the functional core smoke remains incomplete. Subsequent paired-iPhone diagnosis found a Flutter iOS 26 ProMotion implicit-engine startup crash before Dart. The approved explicit-engine correction has since passed repeated physical cold launch, force quit, background, resume, the owner-confirmed 2.8-second reveal, compact policy hierarchy, visible acceptance progress, truthful maintenance denial, and no-crash checks. The first independent review found one bounded policy-recovery defect plus documentation and guard corrections. The independent closure review found no blocker, and its four low findings are closed inside the approved boundary. The corrected complete local matrix passes; packaging and exact-head CI remain. See `docs/EXECUTION.md` for the sanitized receipts. No production retry is prepared, and Gate 4 remains unapproved.

Do not restore the July bundle as blanket rollback. Gate 2 deleted each created-only report trigger and proved it absent before deploying its written-event replacement. Full repair covered all 192 chants and four comments, with 196 complete checkpoints, 196 deterministic audits, and zero projection mismatch. One report whose chant no longer exists remains retained and inert for operator review. The reviewed merge stop is deployed. Reopening, replay, or correction still requires the operational-control and forward-recovery contract.

All 16 source-required composite indexes are ready and reviewed Firestore rules are live. Firebase still has no default Storage resource or Storage rules release. Firebase CLI analysis enabled the Scheduler API during Gate 2, but the two schedule exports are not deployed and there are zero Scheduler jobs. The recovery and deployment buckets are not the application-media bucket and grant no media authority. Media location/provisioning, IAM, retention, resource caps, destructive workers, and schedules remain later approved gates. Firestore and Eventarc are in `nam5`; Functions compute stays `europe-west2`. The data location must not be inferred from the compute region.

Final Gate 3 retry readback found zero performances, drafts, follows, user reports, and deletion or cleanup jobs. Auth still has one enabled verified password account, email signup remains enabled, and no Auth blocking Function enforces a cohort. Both iOS App Attest and Android Play Integrity configurations exist, while Firestore and Authentication App Check enforcement are off. The control is schema version 1, generation 9, mode `maintenance`, and workers false. Every privacy-safe Auth and collection count matched the immediate pre-open baseline. Firestore PITR and delete protection are enabled, and the isolated restore matched all 849 documents. Artifact Registry still has a one-day DELETE/ANY cleanup policy. The launch-policy, deletion, landing, and final hardening source is merged and its non-scheduled Functions are deployed from exact `88ce483f1ea18df6a7a2b4e790803773164ac9a5`. Re-observe all facts before an approved live operation.
...
The active `docs/CHANGE_SPEC.md` is the approved Lane 3 packet from exact `main` at `88ce483`. Its Gate 0 inventory, Gate 1 recovery and containment, Gate 2 cutover, first Gate 3 opening, and three separately approved Gate 3 retry receipts are complete. No Gate 3 attempt produced functional journey evidence. The countdown authority was consumed and safely closed at generation 9. Three device reports proved a native Flutter engine startup crash before Dart, and the approved explicit-engine source correction has physical closed-production proof, including the final presentation calibration. Its independent-review correction and the four low closure-review findings remain local and pass complete local verification. One packaging commit and exact-head CI remain. No production retry is prepared. Media, workers, schedules, public widening, Hosting, DNS, App Check enforcement, stores, and release remain behind later explicit gates. Gate 4 is not next.

Gate 1 created the dedicated V1 runtime identity keyless, granted exactly the reviewed runtime authority, and proved one account-scoped deployer `iam.serviceAccounts.actAs` path. Gate 2 read back all 46 deployed Functions on that identity. Project-wide Editor, Token Creator, and Run Invoker remain absent for it. Re-read these facts before a later deployment; do not broaden them to solve a deploy failure. Missing or changed access is a pre-deploy stop. [Google deployment guidance](https://docs.cloud.google.com/functions/docs/troubleshooting#user_missing_permissions_on_runtime_service_account_while_deploying_a_function).

Gate 1 freshly confirmed the USD 25 monthly budget and five thresholds, measured 0.001833 GiB of Firestore data, and produced a 283,548-byte retained export. These stayed below the approved USD 5 one-time and USD 1 monthly recovery-storage stops. Re-read budget and size before any later material resource or admission widening. Do not treat a budget alert as a spend cap.

### Source operational control, closed in production

The exact private `operationalControls/v1` schema is version 1, positive safe-integer generation, mode maintenance/core/media, and Boolean destructiveWorkersEnabled. No client may read or write it. Gate 1 created generation 1 in `maintenance`; Gate 3 and its retries opened generations 2, 4, 6, and 8 in `core` and closed at generations 3, 5, 7, and 9 in `maintenance`. Workers remained false throughout. Anonymous and authenticated owner reads are denied. Source contains a local read/plan/apply command; this record is not permission to change production again.
...
1. Record current control generation, deployed revision and actual retry/delivery configuration. Inspect delivery failures, retry/backlog or oldest-message evidence where exposed, and worker errors. Gate 2 deployment reported retry-enabled delivery may continue for up to seven days. Verify the deployed policy and history rather than treating a planning reference or CLI warning as an observed guarantee. Quiet logs alone do not prove delivery. [Firebase retry semantics](https://firebase.google.com/docs/functions/retries).
...
Gate 2 completed this procedure on 2026-09-02. The commands below are retained for incident recovery and audit understanding, not continuing authority to repeat production repair.
...
| Cloud Billing budget | Gate 1 freshly confirmed USD 25 monthly; actual 50, 75, 90, 100 percent; forecast 100 percent | Alert-only. Re-read before material resource or admission widening, then review Storage, egress, Functions, and abuse |
| App Check observation | iOS App Attest and Android Play Integrity configured; 1 to 2 weeks of valid release traffic before enforcement | Keep Firestore and Authentication unenforced until legitimate requests are classified and device gates pass |
...
- **Exact single-chant hold point:** A reviewed missing canonical chant must not use the broader club writer. From a clean reviewed head, create an owner-only `.private-report-repair/` directory and run `npm --prefix seed run seed:exact -- plan --project chants-f95b4 --source-sha "REVIEWED_40_CHARACTER_SHA" --credential "/ABSOLUTE/OWNER_ONLY/CHANTS_CREDENTIAL.json" --plan "/ABSOLUTE/REPOSITORY/.private-report-repair/exact-chant-plan.json" --club arsenal.json --chant-id arsenal-north-london-forever --expected-generation 11 --expected-total-chants 192 --expected-team-chants 12`. Review the printed target, expected and required generations, counts, source SHA, and canonical digest. The plan is read-only and refuses source, control, count, target, or team-projection drift.
- **Exact single-chant apply:** Open only the separately approved core generation with workers false. Then run `npm --prefix seed run seed:exact -- apply --project chants-f95b4 --source-sha "THE_SAME_REVIEWED_SHA" --credential "/THE_SAME/OWNER_ONLY/CHANTS_CREDENTIAL.json" --plan "/THE_SAME/PRIVATE/exact-chant-plan.json" --digest "THE_REVIEWED_64_CHARACTER_DIGEST"`. Apply derives all target scope from the owner-only plan, transactionally rechecks control, counts, target absence, and the complete team projection, creates only that document, and requires exact aggregate and target readback. A repeated command, stale plan, conflicting title or ID, permission failure, or changed generation stops. After ambiguous acknowledgement, inspect the same plan and exact readback; never create a new plan or retry blindly.
...
# Durable handoff verification, no device action:
node scripts/prepare-android-device.mjs --apk ../Chants-v1-release-handoff/Chants-1.0.0-1-release.apk --json
# After exactly one unlocked physical Android phone is connected and trusted:
node scripts/prepare-android-device.mjs --apk ../Chants-v1-release-handoff/Chants-1.0.0-1-release.apk --install --json
```
...
The Android candidate helper is intentionally narrower. It is bound to the current signed release APK receipt and accepts only its default build output or one explicit `--apk` path. Before its explicit install mode touches a phone, it rechecks the exact APK hash, package, version, SDK range, complete merged permission inventory, 16 KB ZIP alignment, v2 signature, upload certificate, and rejected advertising permissions. It then requires exactly one authorized physical phone, rejects emulator serials and emulator runtime properties, checks Android 7.0 or later, performs a replacement install, and launches Chants without printing the device identifier or raw ADB output. A replacement install preserves app data only when the installed and candidate signatures are compatible. `INSTALL_FAILED_UPDATE_INCOMPATIBLE` requires an explicit owner decision to uninstall the old app, which deletes its local app data. `INSTALL_FAILED_USER_RESTRICTED` requires enabling the phone maker's USB installation permission instead of weakening the helper. Any candidate rebuild requires a reviewed receipt update before this helper can pass.
...
1. Confirm the backend gate needed for the exact journey is approved and verified. Gate 2 installed `completeOnboarding` and the other non-scheduled source Functions, but maintenance generation 9 currently denies core admission. The explicit-engine iOS correction and final presentation calibration are proven on the paired device, independently review-closed, and locally verified, but are not yet packaged, merged, or exact-head verified. Do not bypass onboarding, create profile documents manually, or reopen production before those gates pass.
