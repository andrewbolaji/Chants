...
| Performances attached to a deleted chant | Redacting a user-authored chant closes current-source visibility and removes its title projection from every dependent performance | A performance owned by another creator also becomes unavailable. That creator's account and media ownership are not deleted by this source-visibility consequence |
