...
**Baseline:** merged main `42f20dc675a1de4fe85956783774a4cdc67f3a01`, all eight jobs green in [exact-main run 33368497566](https://github.com/andrewbolaji/Chants/actions/runs/33368497566). Its tree also matches PR 28's prior green CI. The completed Call-Ups contract remains recoverable at that commit and in its scoped rationale. Neither run includes this documentation-only proposal or future preparation code.
...
| Observation | Core: 30 minutes after successful private smoke. Media: 60 minutes plus the next real daily cleanup and at least two 15-minute monitor executions before public media |
...
1. Create the dedicated runtime account and reviewed narrow grants. Runtime, build and Eventarc invocation are separate identities/permissions. Do not globally strip Editor or remove service-agent roles without an impact inventory. Self-scoped signing must not become project-wide Token Creator.
...
| E1 | `functions:onAccountDeletionJobWritten,functions:onPerformanceMediaDeletionJobWritten,functions:onPerformanceDraftDeleted` | Storage ready; fresh backlog inventory; workers false |
...
Use one dedicated private test account through actual Auth/onboarding, with no manual profile bootstrap. Preserve the existing owner. Synthetic test content only, no destructive tests on seed chants.
...
6. Verify absence of the live object separately from seven-day soft-deleted retention. Observe 60 minutes and the next real cleanup plus two monitor runs and actual alert delivery.
...
Public media remains held on final policy/retention, real cohort scope, operational delivery, terminal cleanup and both configured-device walkthroughs. The zero historical backlog simplifies initial deployment but does not remove ongoing recovery duties.
