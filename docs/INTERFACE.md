...
|---|---|---|---|
| Home | Quiet fan-voiced header, broad search, signed-in Matchday Songbook, simple Premier League entry, then one Terrace Proven and one Chant Lab preview from the existing discovery result. Search returns the existing combined result list. Rising is derived from the current live community chant, never granted by lane membership. An empty trust lane points to real club browse. No persistent navigation, View all, or teamless Add action exists | Condensed display type is reserved for identity and chant names; Nunito carries controls and support copy; Space Mono stays on short labels. Gold and restrained red accents always pair with Terrace Proven, Chant Lab, Original Idea, and truthful Rising words. The page scrolls at 390 by 844 and 1.8x text | `lib/presentation/home/home_screen.dart`, `lib/presentation/browse/discovery_section.dart`, `lib/presentation/shared/chant_card.dart`, `core_journey_golden_test.dart` |
