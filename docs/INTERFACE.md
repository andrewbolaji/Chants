...
| Account deletion | Confirmation states what is removed, what stays anonymized, that the local Songbook is removed, and that completion may continue briefly. Before a request, local data is prepared. An unconfirmed response locks the local Songbook and asks the user to retry without claiming failure or success. After explicit durable acceptance the client signs out. A retained pending session shows only a queued-state explanation and Sign out, never Home or policy acceptance | The recovery state uses text, icon, and one stable action; it scrolls without clipping at 390 by 844 and 1.8x text; request uncertainty and sign-out failures leave a truthful retry path | `lib/presentation/home/home_screen.dart`, `lib/presentation/auth/account_deletion_pending_screen.dart`, decisions 011 and 012 |
...
- **Decision:** An unconfirmed request keeps the local Songbook locked, keeps the user signed in, and says: `We could not confirm whether deletion started. Your Saved Songbook is locked for safety. Try again to confirm the request.` A retry uses the same deletion action. Explicit acceptance keeps the existing queued completion and sign-out flow.
...
- **Accessibility/responsive impact:** The distinction is stated in complete text, delivered through the existing announced snackbar, and does not rely on color or a transient icon alone.
- **Implementation evidence:** Repository and app-gate tests cover ambiguous response, retry acceptance, exact copy, and sign-out boundaries. No hierarchy or control placement changed.
...
- **Implementation evidence:** Service, repository, app-gate, copy, and screen tests plus an inspected 390 by 844 golden. The current complete local Flutter suite passes 322 tests.
...
- **Decision:** Discover keeps its last safe card through an ordinary transient error and removes it on permission denial, current absence, hidden, or removed data. Detail may render route text while waiting, but Save, Share, Report, Vote, and Comment controls require an active, error-free, current visible chant.
- **Why:** Reading public text is reversible and useful offline. Saving, writing, reporting, voting, commenting, and handing text to another application are actions against a target and need current authority.
- **Alternatives considered:** Clear everything on any error, which harms matchday reading; retain every stale action and rely on server rejection, which cannot stop local save or native share; gate only Share, which leaves the interface with inconsistent authority semantics.
...
- **Implementation evidence:** Real Discover permission and transient tests, Discover-to-detail navigation, an all-actions pre-authority widget test, focused comment and vote regression tests, and 282 passing Flutter tests.
