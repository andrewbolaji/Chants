...
| Account deletion | Confirmation states what is removed, what remains as anonymized community or safety history, when the local Songbook locks and is removed, and that completion may continue briefly. Before a request, local data is prepared. Unknown acknowledgement persistently replaces Home with retry and Sign out. Positive acceptance shows queued deletion; a local status-check failure also fails closed. Neither state implies cancellation | Recovery uses text, icon, and stable labeled actions; both screens scroll without clipping at 390 by 844 and 1.8x text; uncertainty, status-check, retry, and sign-out failures keep a truthful next action | `lib/presentation/home/home_screen.dart`, `lib/presentation/auth/account_deletion_recovery_screen.dart`, `account_deletion_pending_screen.dart`, decisions 011, 012, and 016 |
...
- **Decision:** The signed-in gate checks local deletion state before policy or Home. Unknown state shows `REQUEST NOT CONFIRMED`, explains that the Songbook remains locked, and offers `TRY DELETION AGAIN` plus `SIGN OUT`. A local status-check error shows `STATUS CHECK NEEDED` and `CHECK AGAIN`. Positive profile pending state can advance local cleanup and then shows the existing queued-deletion screen. A false pending value never unlocks unknown data.
...
- **Required states:** Unknown at relaunch, another unconfirmed retry, successful retry and sign-out, positive pending reconciliation, local status-check failure and retry, queued deletion, and sign-out failure.
...
- **Implementation evidence:** App-gate, repository, service, screen, and golden tests cover the persistent and positive-only boundaries. Decision 016 and the deletion dialog disclose retained anonymous safety records.
...
- **Implementation evidence:** Service, repository, app-gate, copy, and screen tests plus inspected 390 by 844 pending and recovery goldens. The current complete local Flutter suite passes 336 tests.
...
- **Implementation evidence:** Real typed-permission and transient Discover tests, Discover-to-detail navigation, existing-save removal and local-club navigation tests, an all-actions pre-authority widget test, focused comment and vote regressions, and 336 passing Flutter tests.
