...
**Not yet done:**
- **Immediate prerequisite, observed 2026-08-30:** Bring the live backend up to reviewed source before the new-account/device walk. The approved Node 22 readiness change passes production build and all 163 Functions tests locally. Production still has nine July Functions versus 48 source exports, two unsafe report-trigger overlaps, old rules, fourteen missing indexes, no media bucket, and disabled Storage/Scheduler services. `docs/changes/2026-08-30-v1-backend-rollout-readiness.md` records the actual predecessor and named future groups. Packaging and exact-head CI are authorized. The next deployment-safety specification needs approval before implementation; include that final source in one consolidated Claude review before the separate production amendment. The owner completed development-certificate setup and one valid signing identity is verified; provisioning and the device walk remain. The catalogue itself is already exact.
...
The next Claude review spans last-reviewed `cb50d3cc966c6a367309c887a8c765891155cf0e` through the eventual packaged backend-readiness head: PRs 22-24 plus this block. Include the engineering overview, implementation rationale, seed records, readiness/cutover record, runbook, and exact-head CI. Previous optional Living Songbook findings are carried in the readiness record. Do not postpone this source/decision review until after an irreversible production cutover. Deployment and device results receive a subsequent evidence closure.
