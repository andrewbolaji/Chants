...
- The approved V1 club-identity, vote-clarity, and Arsenal Songbook correction passed exact-head CI, consolidated review, exact create-only production seeding, and its paired-iPhone walk. Club Signal uses the Chants ink, supporter-gold, and warm-paper family; all 20 hash-bound local crests were owner-approved without distortion; vote controls name no vote, up, and down while preserving the existing net-score rule; and North London Forever is live with only the short reviewed supporter refrain. Production is independently read back with 193 chants, 13 for Arsenal, and generation 13 maintenance with destructive workers false. The remaining amended-candidate work is the approved player-named Call-Up copy correction, its exact-head CI, and its limited phone presentation check.
...
- **Next, updated 2026-09-11:** The Storage-to-Firestore Rules bridge and generation-34 iPhone media canary passed. Pull request 38 then passed consolidated review, accepted-finding closure, signed native rebuilds, and exact-head run `34612075347` at `3ddcad2`. Andrew subsequently approved one bounded first-run orientation extension before merge. Its 45 focused tests, 588-test complete Flutter suite, clean analysis, inspected goldens, and fresh signed Android and iOS receipts pass. Put only that extension into pull request 38, require exact-head CI, and complete the rebuilt-phone presentation check. That walk must cover the orientation exits, simplified launch reveal, clearly separated public `@handle`, iOS picker behavior, blocking upload state, immediate cancellation feedback, and the remaining release journeys. No merge, store upload, or submission follows without its applicable owner authority. The maintained owner checklist is `docs/CHANTS_LAUNCH_COMMAND_CENTER.html`.
