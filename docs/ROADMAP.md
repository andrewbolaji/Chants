...
- Stable seeded chant identity in source: explicit immutable IDs, collision preflight, and transactional ownership checks. Named-project preflight is clean for all 192 targets; production reconciliation and rollout remain pending.
- V1 Premier League catalogue source: complete for all 20 approved clubs. The files contain 192 chants and 622 reviewed squad rows, dated squad and source metadata, explicit current or historic treatment, and named owner overrides where the refreshed official feed lags reviewed Arsenal membership. Local validation and currentness pass. Named-project preflight is clean for all 192 chant targets; no Firebase write has occurred.
...
- Guarded Arsenal live reconciliation, controlled club writes, and exact post-write readback for the source-complete 20-club catalogue. The transfer-sensitive 20-club roster review and named-project identity preflight are complete.
...
The repository uses explicit seeded chant IDs and freezes Arsenal to its expected legacy IDs. The owner review is complete, and source now contains all 20 approved clubs. The 2026-08-30 refresh compares 623 raw official-feed rows to 622 reviewed rows after 17 display aliases and three named Arsenal membership overrides. Named-project preflight is clean for all 192 targets. The baseline found only the expected absent 19-club catalogue plus Arsenal's four additions, three unreferenced departures, and 12 legacy chants missing `origin`. The guarded Arsenal reconciliation must be reviewed and exact-head green before the Leeds canary or wider production writes.
