...
- PR 16 and its reviewed correction are merged to `main` at `86603c22fbd7647f89c9276af9a60a0b3d63113b`. PR 17 packages Product Clear navigation, public creator identity, Chant Stage, moderated 30-second performance upload, popularity, public destinations, private follows, activity, mentions, continued performance replies, media safety, and its accepted takedown correction. PR 18 adds launch authentication, recoverable onboarding, and Android source readiness. Exact PR 18 head `db40f42` passed all eight clean-runner jobs in run `33206487262`; its consolidated independent review found no source-freeze blocker. The approved nine-finding post-auth correction passes 463 Flutter, 142 Functions, 42 seed, analysis, rules TypeScript, and governance checks locally and awaits packaging plus replacement CI.
...
**Not yet done:**
- Package the locally verified post-auth correction and require replacement eight-job exact-head CI, including its new Storage authority assertions.
- Complete provider dashboards, credentials, association files, production signing, policy, cost and abuse controls, and the combined iOS and Android V1 device walkthrough. Exact PR 18 base compilation already passes on both clean runners.
...
14. **Integrate and freeze the creator expansion.** Packaged, clean-runner green, and independently accepted. The correction closes current creator and chant authority, source reconciliation, exact performance counts, Stage and profile blocking, hidden-content escalation, and durable media deletion.
15. **Build launch authentication, onboarding, and Android readiness.** Packaged through exact PR 18 head `db40f42`, clean-runner green across eight jobs, and independently accepted with no source-freeze blocker. Chants has a product-specific welcome, verified email and automatic return, recoverable server-owned onboarding, Apple, Google, Facebook, magic-link and phone paths behind fail-closed flags, explicit same-UID linking, truthful reset, native entitlements and paths, fail-closed Android signing, and native CI.
16. **Close post-auth review findings.** Implemented and locally verified. The nine corrections align Storage operator authority, retry provider initialization, restore onboarding escape paths, make phone cooldown and cancellation monotonic, retain ambiguous magic-link state, distinguish requested from connected or complete, and enlarge the onboarding destination target. Replacement exact-head CI remains before source freeze.
...
The pre-creator V1 stack is merged through `86603c2`. PR 17 carries its reviewed creator correction. PR 18 exact head `db40f42` passed eight-job run `33206487262`, including Android and iOS builds, and the consolidated review found no source-freeze blocker. The local post-auth correction still needs packaging and replacement exact-head CI. Provider setup, production signing, association deployment, policy, cost controls, and the combined device walkthrough remain gates that turn source confidence into release sign-off.

**Trigger to exit:** The post-auth correction is exact-head green; both native clients still compile; recording or choosing, upload recovery, approval, playback, sharing, Following, comments, activity, moderation, blocking, deletion, authentication, verification, and offline Songbook pass the combined device walk; provenance and promotion rules still work end to end; and public share-out never produces a broken destination.
...
- **IN PROGRESS** Launch authentication and onboarding are packaged, independently reviewed, and exact-base clean-runner green with server-owned initial profile creation and requested providers hidden until configuration. The local nine-finding correction, replacement exact-head CI, provider setup, and device evidence remain before release.
