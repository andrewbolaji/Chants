...
- The approved V1 club-identity, vote-clarity, and Arsenal Songbook correction passed exact-head CI, consolidated review, exact create-only production seeding, and its paired-iPhone walk. At that milestone Club Signal used the Chants ink, supporter-gold, and warm-paper family; all 20 hash-bound local crests were owner-approved without distortion; vote controls named no vote, up, and down while preserving the existing net-score rule; and North London Forever went live with only the short reviewed supporter refrain. The later completed release-candidate walk approved a dark in-app Club Signal correction, recorded below. Production is independently contained at the later generation-35 boundary recorded in the live rollout hold.
- The approved V1 store submission presentation packet is prepared in PR 35 from PR 34 exact green head `b952389e20b7c510f204f5cbbeb87af70b4c2574`. Independent-review corrections bind source, asset, release, and screenshot evidence to hashes; reject placeholder captures; account for the linked Meta SDK; and make Apple V1 iPhone-only. Andrew approved the final Google feature-graphic bytes on 7 September 2026. The packet remains `prepared_not_submitted`: final captures, live URL and support readback, review access, signing, exact binaries, console entry, questionnaires, submission, and release are not claimed.
...
- **Next, updated 2026-09-11:** The Storage-to-Firestore Rules bridge and generation-34 iPhone media canary passed. Pull request 38 then passed consolidated review and accepted-finding closure. Andrew completed the rebuilt-phone presentation walk, accepted the refined three-page onboarding, and confirmed that the retained account now signs in and enters Chants quickly. Current source has horizontally swipable charcoal orientation with softly faded full-panel dots and bottom-anchored action phrases, one official supporter-shield mark on the signed-out welcome, bounded credential and signed-in root gates, one quiet automatic profile retry before specific fail-closed recovery, dark in-app Club Signal, a content-sized posting identity, and a compact Terrace Proven badge explained as accepted evidence of being sung at matches. The complete local Flutter suite passes all 596 tests, scoped analysis is clean, and the store validator plus all 20 regressions pass. Current signed Android AAB and APK artifacts pass identity, integrity, permission, and upload-certificate continuity checks. Android platform-tools and a fail-closed exact-candidate installer are prepared so the physical Android session can begin with one authorized phone and proceed directly to the six-step walk. Pull request 38 passed all eight exact-head jobs in run `34667543812` at `3066474`; its later commit changes only documentation, tests, and inspected Linux visual references, not app runtime or native source. The current App Store archive and IPA now pass identity, archive, distribution-profile, entitlement, and strict signature checks without upload. All three artifacts are preserved in a durable local handoff folder with exact hashes. Refresh the public site's older light Club Signal illustration before using it as final current-app evidence. Physical Android, store-installed beta walks, provider activation, public routes, final captures, console entry, submission, merge, and release remain separately gated. The maintained owner checklist is `docs/CHANTS_LAUNCH_COMMAND_CENTER.html`.
- **Live rollout hold:** Production has exactly 46 non-scheduled Node 22 Functions from one reviewed source build plus the separately deployed dormant cleanup schedule. The two report migrations completed without overlap; 196 repair checkpoints and audits have zero projection mismatch. Maintenance generation 35 is active with destructive workers false. The Firebase-linked app-media bucket retains exactly one private staged canary object whose path and metadata match one pending-review draft. No upload grant, public performance, media-deletion job, or deferred cleanup job exists. The documented cross-service bridge is present, while Storage, Firestore, and Authentication App Check remain unenforced pending later traffic evidence and separate approval.
- **Safe preparation in parallel:** `scripts/check-device-readiness.mjs`, the exact-candidate Android helper, and the private command-center guide provide local inventory, guarded installation, and per-platform walkthrough capture. Current Android and iOS distribution binaries are preserved in a durable local handoff folder with exact hashes. Browser visual proof remains open. No runtime feature, live service, store, or production change is included.
- Complete the separately gated release provider setup for Sign in with Apple, Google, Facebook, passwordless email, and email-password. Provider buttons stay hidden until their dashboard, credential, callback, entitlement or URL scheme, domain-association, and configured-device checks pass. Then finish Android association, deployed Apple association, production signing, policy, observed cost and abuse controls, and the combined iOS and Android V1 device walkthrough.
...
The repository uses explicit seeded chant IDs and freezes Arsenal to its expected legacy IDs. The owner review is complete, and source contains all 20 approved clubs. The 2026-08-30 refresh compares 623 raw official-feed rows to 622 reviewed rows after 17 display aliases and three named Arsenal membership overrides. Named-project preflight was clean for all 192 production targets. PR 23 merged the reviewed controls after all eight exact-head CI jobs passed. The bounded production sequence then created Arsenal's four additions, reconciled all 12 production chants, removed only the three exact zero-reference departures, and finished with one matching team, 28 matching players, 12 matching chants, and zero missing, mismatching, or orphan rows.

The earlier prepared catalogue had 192 chants in total, including 180 across the 19 new files. The approved 8 September source correction added one Arsenal club chant, bringing source and production to 193. Every new club clears the three-chant floor, and larger supported songbooks were retained instead of applying the old five-chant cap. Content integrity remains fail-closed: correct source JSON rather than improvising a lyric, player, context claim, or evidence link during rollout.

The stable-ID and current-roster gates apply to live data writes, not only the completed source catalogue. Arsenal reconciliation and Leeds canary passed their separate hold points. The approved six-group widening sequence then completed with exact readback after every group. The later create-only North London Forever operation passed its clean-head, CI, review, preflight, exact readback, device walk, and closure gates. Final production verification reports 193 chants, 13 for Arsenal, the exact new target, unchanged non-target counts, and generation 13 maintenance with workers false.
...
19. **Orient a first-time supporter without adding friction.** Approved as a bounded pull request 38 extension. A signed-out installation gets three skippable product truths for Songbook, Chant Lab, and Stage, plus a direct Create Account path and the existing sign-in welcome. One versioned local boolean prevents repeat display. Authentication, verified-account profile setup, account authority, and guest-access policy remain unchanged. Focused interface and failure proof, the complete local Flutter matrix, inspected references, and fresh signed native receipts pass. Exact-head CI and rebuilt-phone presentation remain open.
...
The operator-seeded Songbook makes the app useful on day one. The first-run guide explains Songbook, Chant Lab, and Stage once without forcing account creation inside the guide, while keeping both Skip and a direct Create Account route visible. Chant Lab, player creation prompts, voting, comments, sharing, and Saved Matchday Songbook provide creation, competition, conversation, distribution, and matchday return loops without waiting for a personalized feed.
...
### 12. Quick biometric or device-credential return

- **Status:** Pinned for V1.1 by Andrew on 11 September 2026. It is not part of the current V1 release candidate.
- **Purpose:** Let returning supporters unlock a securely retained session with Face ID, Touch ID, Android biometrics, or the operating system's device credential when appropriate.
- **Smallest first slice:** Evaluate an operating-system biometric or device-credential prompt around a securely retained authenticated session, with the existing sign-in and account-recovery paths as fallback. If passkeys are feasible with the selected Firebase identity architecture, assess them separately as a true passwordless sign-in option.
- **Safety boundary:** Never store or process biometric data, never store a user's password, and never create a Chants-only PIN. Authentication and fallback credentials remain owned by iOS or Android secure storage and system prompts.
- **Success signal:** returning users reach their account faster without increasing lockouts, recovery failures, account ambiguity, or support load.
- **Implementation gates:** threat model, session-expiry behavior, account switching, device-loss and recovery handling, platform capability review, accessibility, explicit opt-in copy, and physical iOS and Android proof.

## Post-V1 exploration: Supporter timeline

- **Product direction:** Let supporters publish short football thoughts, follow other supporters, visit public profiles, and repost posts into a personalized timeline. Chants and chant performances remain the product's distinctive center; the timeline exists to create a daily football conversation loop around that identity rather than turning the app into an undifferentiated X clone.
- **Smallest exploratory slice:** Text-only posts from existing public creator identities, optional links to a club, player, fixture, chant, or performance already known to Chants, a following-only chronological timeline, profile post history, replies, likes, and one clearly attributed repost action.
- **Why it may work:** Songbook use is naturally concentrated around matchdays and discovery moments. Lightweight football conversation can create reasons to open Chants between fixtures, then route attention back into chants, performances, Call-Ups, and club communities.
- **What must remain separate:** A supporter opinion is not chant evidence, Terrace Proven status, an official club statement, or a moderation signal. Reposts are attribution and distribution, not endorsement or proof. Existing chant trust labels and ranking cannot be reused as post truth or popularity authority.
- **Safety and operations gate:** Before implementation, decide server-owned post admission, edit and deletion behavior, reply depth, quote-post treatment, repost deduplication, block propagation, report categories, rate limits, moderation queues, notification fan-out, link safety, search or discovery eligibility, retention, account deletion, and cost ceilings. Text makes volume and moderation materially broader even without media.
- **Success signals:** Weekly posting and reading retention, healthy follow-to-timeline activation, meaningful routes from posts into chant or club surfaces, balanced creator distribution, low block/report rates, and a moderation workload the launch operation can sustain.
- **Keep out of the first slice:** Algorithmic For You ranking, trending topics, direct messages, spaces or live audio, anonymous posting, external embeds, link previews fetched by the server, monetization, ads, paid boosts, and unmoderated image or video posts.
- **Decision trigger:** Revisit after V1 has real retention and moderation data. Prototype only if supporters return for club and creator activity but lack a lightweight way to talk between chant-specific actions. This is not a V1 or promised V1.1 feature.
...
- **DONE IN SOURCE, FEATURE GRAPHIC APPROVED** The 1024 no-alpha App Store icon source exists, a validated 512 alpha-bearing Google Play icon is in the store packet, and the final 1024 by 500 Google feature graphic has exact-byte owner acceptance. Real screenshots, platform previews, and release-binary asset readback remain.
