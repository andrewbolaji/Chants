...
**Not yet done:**
- **Immediate prerequisite, updated 2026-08-31:** Bring the live backend up to reviewed source before the new-account/device walk. PR 25 readiness is exact-head green at `d7b8b6f`. The approved deployment-safety working tree now adds admission, two-lookup upload permission, retained cleanup and bounded report repair. Packaging and exact-head CI are authorized, followed by one combined Claude review; PR 25 records the immutable result. Production remains the nine-Function July baseline with old rules, two report event-type migrations, fourteen missing indexes, no media bucket and disabled Storage/Scheduler services. The separate production amendment must prove containment/drain, precise replacement, full repair/readback and bounded retained-job replay before admission opens. The catalogue and development-certificate setup are complete; provisioning and device evidence remain.
...
The next Claude review spans last-reviewed `cb50d3cc966c6a367309c887a8c765891155cf0e` through the final safety head after authorized packaging and exact-head CI: PRs 22-25 plus the deployment-safety block. Include overview, repository/scoped rationales, seed records, decision 026, runbook and exact-head CI. Previous optional Living Songbook findings remain in the readiness record. Do not postpone this source/decision review until after a production cutover. Deployment and device results receive a subsequent evidence closure.
