...
- **Next, updated 2026-09-11:** The Storage-to-Firestore Rules bridge and generation-34 iPhone media canary passed. Freeze the current source, rebuild Android and iOS candidates, complete the consolidated review from Claude's PR 37 pre-correction tree through that frozen candidate, close accepted findings, and rerun exact-head CI plus native release checks. The rebuilt phone walk must verify the simplified launch reveal, the clearly separated public `@handle`, iOS picker behavior, blocking upload state, immediate cancellation feedback, and the remaining release journeys. No store upload or submission follows without separate owner authority. The maintained owner checklist is `docs/CHANTS_LAUNCH_COMMAND_CENTER.html`.
