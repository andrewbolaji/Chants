...
**Not yet done:**
- Package the final minor onboarding and evidence closure and require replacement eight-job exact-head CI before merging PR 17.
...
17. **Close final source-freeze minor findings.** In progress. Saved onboarding values become immutable while profile projection catches up, and engineering records reflect exact combined-head evidence. Replacement exact-head CI remains before merge.
...
The pre-creator V1 stack is merged through `86603c2`. Combined PR 17 head `5350b8a` contains the reviewed creator, authentication, Android, and post-auth correction source. Run `33215692105` passed all eight jobs there, including both native builds and 165 Java-backed rule cases. This final minor closure needs replacement exact-head CI before the stack merges. Provider setup, production signing, association deployment, policy, cost controls, and the combined device walkthrough remain gates that turn source confidence into release sign-off.
...
- **IN PROGRESS** Launch authentication, onboarding, the nine-finding correction, and Android source readiness are packaged, independently reviewed, and exact-head clean-runner green with server-owned initial profile creation and requested providers hidden until configuration. The final minor closure, provider setup, and device evidence remain before release.
