...
### Early v1.1: System, light, and dark appearance
**The idea:** Make appearance the first presentation fast follow after V1. Offer System, Light, and Dark choices. First launch follows the device; an explicit choice persists locally until the user changes it.
**Why early v1.1:** V1 ships in one inspected dark appearance. Light mode improves daytime and bright-stadium comfort, personal preference, and accessibility, but it should not invalidate final V1 screenshots or reopen the store submission candidate. It moves into the first V1.1 block instead of waiting for 30 days of use.
**Design direction:** Preserve Chants as a supporter and matchday product in both appearances. Dark keeps the current restrained ink, gold, coral, cream, and blue hierarchy. Light uses warm paper and cream surfaces, ink text, the same controlled accents, and no generic white-card or gradient treatment. Both modes must flow through semantic tokens in `colors.dart`, `theme.dart`, and `spacing.dart`; screens do not own raw theme colors.
**Proof required:** Cover Stage video and fallback, Club Signal, chant detail and lyrics, Create, Songbook, You, auth, policy, report and moderation states, loading, empty, offline, errors, keyboard, sheets, dialogs, snackbars, and 1.8x text. Verify WCAG AA contrast for body text and meaningful controls. Add focused token and persistence tests plus representative visual evidence without duplicating every widget test.
**Release rule:** Any appearance or token change after store capture invalidates affected screenshots. Begin after the exact V1 release candidate and its store images are locked, then ship as the earliest V1.1 presentation block.
