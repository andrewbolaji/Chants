...
| Functions production build and tests | `npm ci && npm run build && npm test` | `functions/`, Node 22 | Production `lib/index.js`, 229 passing unit tests and 24 intentionally skipped emulator cases without the demo emulator |
...
| Current launch-policy and deletion closure exists only in the local source boundary | The placeholder contract remains live until this source is independently reviewed, merged, published, deployed, and observed | Andrew | Before any backend cutover, core walkthrough, or policy publication |
