...
- **Secrets:** Admin credentials and native Firebase configuration are not committed. Production secret storage and rotation are dashboard-owned and not verified from source. The live Firebase Admin SDK account has two user-managed keys whose identifiers and owners are intentionally absent from project memory; their exact disposition is a cutover hold.
...
- **Environments:** Local and emulator are evidenced. The read-only 2026-09-01 refresh confirms production is behind source: nine July Node 20 Functions in `europe-west2`, Firestore and event location `nam5`, old Firestore rules, two of sixteen required indexes, no operational control, no Firebase app-media bucket and Scheduler disabled. No repository-defined staging environment exists.
...
- **Runtime source settings:** `functions/src/runtime_options.ts` selects the dedicated V1 runtime account, CPU 1, min 0, max 3/concurrency 20 generally, and seven max-1/concurrency-1 exceptions. Worker timeout/memory is 300s/512 MiB; other timeouts remain 60s. The account/grants and resource changes are not deployed. See the preparation rationale for the complete manifest and live holds.
...
- **Backup or restore evidence:** PITR and database delete protection are disabled, and no backup or restore-exercise evidence exists. Only `(default)` exists; the proposed `chants-rollout-restore-20260901` name is unoccupied but no creation, export, import, access proof or cleanup is authorized. This is a release-readiness gap.
...
|---|---|---|---|
| Nine deployed Functions versus 48 source exports; two created-to-written report cutovers; predecessor has blind increments and active merge | No overlap or blanket rollback is safe. PR 25 is merged at 72a39b1; PR 26 correction head 2d362a2 has eight-job exact-tree CI and independent F1-F5 closure, not deployment | Andrew | Separate production containment/drain and cutover approval after source merge |
...
| Cloud Storage API is enabled, but the Firebase app-media bucket and Storage rules release are absent; Scheduler is disabled; fourteen indexes are missing | Video and scheduled operations cannot work against the live baseline | Andrew | Approve exact Firebase bucket provisioning, services/IAM, index readiness, resource/retention settings, and separate destructive-worker activation before admission |
...
| Combined device walk is incomplete | A configured physical iPhone now reaches the content-policy gate and a development-signed release build launches. Policy acceptance fails closed because the required backend rollout is still unavailable; Android and the remaining candidate-bound journeys are unverified | Andrew | Complete backend admission and the combined iOS/Android walkthrough before V1 sign-off |
...
| Current launch-policy, deletion closure, and public landing page exist only in source | The live origin and client remain unchanged until these sources are independently reviewed, merged, published, deployed, and observed | Andrew | Before any backend cutover, core walkthrough, policy publication, or domain launch |
