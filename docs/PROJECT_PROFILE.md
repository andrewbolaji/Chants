...
| Functions production build and tests | `npm ci && npm run build && npm test` | `functions/`, Node 22 | Production `lib/index.js`, 214 passing unit tests and 23 intentionally skipped emulator cases without the demo emulator |
| Seed tests | `npm ci && npm test && npx tsc --noEmit` | `seed/`, Node 20 | Mocha results and TypeScript type check |
| Firestore and Storage rules | `npm ci` in `test_rules/`, then `firebase emulators:exec --only firestore,storage --project chants-f95b4 "cd test_rules && npm test"` from the repository root | Java, Firebase emulator; CI Node 22 | 173 rule and source-budget checks with synthetic fixtures |
| Control, report repair and upload transactions | `firebase emulators:exec --only firestore --project demo-chants-repair "cd functions && npx mocha --timeout 60000 lib-test/test/report_repair.integration.test.js"` after Functions test build | Java 21, Node 22; fixed local emulator only | 23 real transaction cases, no credentials or production |
...
- **Operational private data:** Control, upload grant, deferred cleanup paths and report-repair checkpoints are server-owned. Legitimate cleanup evidence retains owner-containing paths without TTL, including after account deletion. Permanent identity/path faults retain minimal blocked evidence with a validated draft locator in the same private collection. Both require separately approved recovery/retention and are not public creator data. Maintenance gates app work, not Firebase Auth sign-in/reset itself.
...
| Paused cleanup retains exact private paths but has no automatic historical replay or deferred-cleanup monitor | Mode changes do not process old jobs; attempted cleanup is not permanent absence, and missing event delivery can leave no row to monitor | Andrew | Before workers/media open: runbook's manual delivery/object reconciliation and recovery/retention gate; before public media: demonstrated ongoing observation owner/cadence |
