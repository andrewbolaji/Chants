...
- **Name:** Chants
- **Purpose:** A mobile football songbook for learning Terrace Proven chants and a Chant Lab where supporters can create, discuss, and rank what might be sung next.
...
- **Non-goals for V1:** Hosted audio or video, background media, unlimited reply nesting, reply notifications, creator follows, scheduled challenges, cloud sync for Saved Matchday Songbook, public chant URLs, and automated lyric generation.
...
| One direct comment reply level | Keeps V1 conversation usable without orphan or deep-thread complexity | Rules, repository, widget, and moderation tests |
...
- **Data classes:** Public chant, team, player, comment, and selected profile display data; private per-user votes, likes, blocks, reports, feedback, local Songbook snapshots, safety budgets, deletion jobs, and operator audit data; restricted credentials and Admin access remain outside the repository.
...
- **External integrations:** Firebase Auth, Firestore, Functions, App Check, and Crashlytics; operating-system share sheet; allowlisted YouTube or X evidence links through the system browser. Timeout, retry, current-authority, and idempotency behavior is documented in the repository rationale and decisions.
...
- **Resource or cost budget:** No formal Firebase budget is recorded. Bounded local snapshots, bounded safety windows, and bounded deletion pages limit obvious growth paths.
