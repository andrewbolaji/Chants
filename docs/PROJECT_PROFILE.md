...
| Functions production build and tests | `npm ci && npm run build && npm test` | `functions/`, Node 22 | Production `lib/index.js`, 205 passing unit tests and 18 intentionally skipped emulator cases without the demo emulator |
...
| Report repair and upload transactions | `firebase emulators:exec --only firestore --project demo-chants-repair "cd functions && npx mocha --timeout 60000 lib-test/test/report_repair.integration.test.js"` after Functions test build | Java 21, Node 22; fixed local emulator only | 18 real transaction cases, no credentials or production |
...
- **Feature flags or staging:** One private operational interlock, not a general flag platform: maintenance/core/media plus destructive-worker enablement. Missing/invalid control closes protected work. Admin must increase generation on every transition. No control writer, live transition or historical replay is authorized by source approval. `mergeChants` remains stopped regardless of mode.
