...
- **Current or target workload:** Pre-launch. Reviewed source contains 192 chants and 622 squad rows across all 20 approved Premier League clubs. Named-project preflight is clean for all 192 chant targets. Arsenal is the only club live; exact Arsenal reconciliation, Leeds canary, controlled widening, and post-write readback remain.
...
| Only Arsenal is live; all 192 chant targets passed named-project preflight, but exact Arsenal reconciliation and the controlled rollout remain | Most clubs have no confirmed canonical live primer set until exact readback succeeds | Andrew | Before the first production seed write and after every bounded rollout group |
