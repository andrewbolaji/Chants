...
| Functions production build and tests | `npm ci && npm run build && npm test` | `functions/`, Node 22 | Production `lib/index.js`, 202 passing unit tests and 12 intentionally skipped emulator cases without the demo emulator |
...
| Report repair and upload transactions | `firebase emulators:exec --only firestore --project demo-chants-repair "cd functions && npx mocha --timeout 60000 lib-test/test/report_repair.integration.test.js"` after Functions test build | Java 21, Node 22; fixed local emulator only | 12 real transaction cases, no credentials or production |
...
- **Operational private data:** Control, upload grant, deferred cleanup paths and report-repair checkpoints are server-owned. Cleanup evidence retains exact paths for separately approved replay/retention; it is not public creator data. Maintenance gates app work, not Firebase Auth sign-in/reset itself.
...
|---|---|---|---|
| Nine deployed Functions versus 48 source exports; two created-to-written report cutovers; predecessor has blind increments and active merge | No overlap or blanket rollback is safe. PR 25 is merged at 72a39b1 and safety source 421463e is exact-head green, but not independently reviewed or deployed | Andrew | Combined review including local preparation, then separate production containment/drain and cutover approval |
