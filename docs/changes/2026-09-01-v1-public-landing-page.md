...
The two Hosting font binaries are mechanical copies of already tracked files under `assets/fonts/`. SHA-256 comparison must confirm each destination is byte-identical to its source before handoff. No new font download or uncertain asset provenance enters the repository.
...
Andrew approved `PR 31 post-review and operator-name correction spec` after an independent read-only review of `75e1bc3...e9ee728`. The correction adds a real app-level timer-handoff regression, preserves a truthful progress cue when initialization outlasts the one-shot reveal, and adapts the reveal for compact landscape and enlarged text. The agreement action now wraps without shrinking required copy and remains inside the small viewport. Public product illustrations expose one useful label without leaking simulated headings or navigation to assistive technology; their visible shell matches the real five-tab product navigation, and the play action no longer covers the supporter mark. Reviewed contrast and focus pairs are corrected in source.

The exact operator spelling is `ThunderRiver Tech LLC` across the root, all six public policy and support pages, app policy copy, tests, launch records, and launch guidance. The correction does not alter policy meaning, policy version, the 17+ rule, authentication or backend authority, Firebase rewrites, DNS, deployment, or store availability.

Post-review verification passes 32 focused Flutter tests across the app gate, launch reveal, and current-policy gate; 524 complete Flutter tests; clean Flutter analysis across `lib` and `test`; 18 focused landing and launch-policy Node tests; all 37 device, guide, policy, and landing Node tests; and the launch-services contract plus its regression test. Staged governance, replacement exact-head clean-runner, and targeted independent-review evidence remain required before merge or deployment.

## Final compact loading correction

The targeted closure review confirmed the earlier findings closed and reproduced one remaining low-severity intersection defect. The launch reveal overflowed only when unresolved progress, compact geometry, and enlarged text were active together. Andrew approved `PR 31 final compact loading correction spec` for that exact boundary.

Two production-widget regressions were added before runtime source changed. They failed on the reviewed head with a 37-pixel overflow at 568 by 320 with 2x text and a 19-pixel overflow at 320 by 480 with 1.5x text. The corrected reveal treats the short portrait as compact, passes compact state into the detail block, and uses a spinner-only visible loading cue in compact geometry. A normal portrait retains the written cue. Both forms keep the complete `Chants. Getting things ready.` semantic announcement. All six focused launch-reveal tests, all 28 combined app-gate and launch tests, the complete 526-case Flutter suite, and CI-equivalent analysis pass after the correction.

This correction changes no launch timing, navigation, policy meaning, policy version, 17+ rule, authentication or backend authority, Firebase rewrite, Hosting source, DNS, deployment, or store state. Project memory, writing style, governance regressions, native-project, launch-services, all 37 device, guide, policy, and landing Node cases, and whitespace checks pass. One correction commit, push, and replacement exact-head clean-runner CI remain required before merge or deployment.

## Owner acceptance and remaining gates

Andrew granted visual acceptance for the final public direction on 1 September 2026. The first independent review found bounded launch and accessibility defects, and Andrew approved their correction. The targeted closure review confirmed those findings closed and identified the final compact loading defect recorded above. Complete verification, one correction commit, push, replacement exact-head clean-runner CI, merge, gated Hosting deployment, DNS/domain connection, live root and trust-route readback, rich-preview cache verification, and verified store buttons remain separate. The live origin is unchanged until rollout evidence says otherwise.
