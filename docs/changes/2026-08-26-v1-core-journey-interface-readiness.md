...
The new CI job has not run on GitHub yet. Final merged `main` still has five passing jobs in run `32993748570`; this branch proposes a sixth governance job and requires clean-runner evidence after packaging.
...
| New governance CI has no remote run yet | Linux executable and workflow behavior are locally inspected only | Packaging and clean-runner CI are the next publication gate | Codex | Branch push and pull request |
