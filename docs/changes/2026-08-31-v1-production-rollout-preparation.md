...
- **Base:** Main `42f20dc675a1de4fe85956783774a4cdc67f3a01`. **Target:** Preparation was committed and independently reviewed at `5280c3ab9c41f27f0836c1f8940a23c5ecc35b1b` on PR 29. The approved documentation/comment closure starts at that head; its final SHA, tree and replacement CI belong in the PR receipt, not the predecessor's green result.
- **Approval:** The source-preparation portion of `docs/CHANGE_SPEC.md` is approved. Andrew subsequently authorized packaging, then all five proposed review clarifications together in one correction commit, push to PR 29 and replacement CI. Cloud writes, credential access, deployments, control transitions, repair, paid rehearsal and merge remain excluded.
...
Apply checks the approved digest/source before database access. A transaction reads only `operationalControls/v1`, compares data and update time, then creates or replaces only that document. The SDK makes at most three transaction attempts including the initial attempt, so at most two retries. Version matching rejects an intervening change-and-restoration even when the four fields match again. A literal no-op set can preserve the timestamp because it did not change stored data.

An already-matching target is observed without another write. After either acknowledged execution or a non-stale transaction exception, a separate read must match the exact target. The output deliberately says `target-observed`, not that this plan authored the state or proved write permission. A competing writer could have produced the same target. Readback validation/read errors can replace the original transaction cause; raw SDK errors remain private. A stale different target stops; missing/invalid/read-failed state never becomes success. There is no compensation, automatic next transition, rollback, queue replay or generation repair.
...
|---|---|---|
| Candidate | Final correction commit/tree and built endpoint/source hashes | Preparation at `5280c3a` has eight-job green CI and independent review; the comment/document correction requires its own immutable candidate and exact-head CI receipt |
...
| Deployer and control/repair principal | Exact private identities intentionally unresolved | Explicitly verify the deployer's effective `iam.serviceAccounts.actAs` on `chants-v1-runtime@chants-f95b4.iam.gserviceaccount.com` before deploy, using the reviewed account-scoped binding and current private permission evidence. No broad fallback grant or automatic grant to control/repair identities. Separately approve temporary least-privileged control/repair access and private credential handling; neither CLI accepts a Firebase CLI login as its service-account file. No shared Admin key exemption by convenience |
...
| Schedules/workers | Existing source daily 03:00 UTC cleanup and 15-minute monitor | Enabling/deploying Scheduler may activate immediately. Keep workers closed and inventory jobs/terminal drafts. Andrew owns the runbook's manual deferred-cleanup/delivery gate; the monitor covers only account/media jobs, not deferred rows or never-persisted events. Fresh nonzero backlog requires exact recovery; public media needs a sustainable observation plan |
| Private test scope/capacity | Owner preserved; disposable accounts through normal Auth/onboarding | Inventory actual signup/provider/Auth scope and approve actual admission restrictions or exposure. Approve an additional disposable account for the runbook's concurrent-submission probe, then workload/budgets and observed cap decision before public media. Project-wide flags are not a per-UID canary; source limits alone do not prove capacity |
...
- No new Flutter/native/rules behavior exists in this diff. The preparation passed exact-head CI and independent review as recorded below; the follow-up needs replacement CI. No production smoke, IAM probe, capacity test or backup/restore rehearsal has run.

## Independent review and documentation closure

Claude reviewed `42f20dc..5280c3a` and found no code/merge blocker. It independently compared all 48 base/head compiled endpoint manifests, reproduced the resource regression against base, re-ran 214 unit tests and reproduced both compiled artifact hashes. Its local environment used Node 20 and could not run Java-backed integration; those limits are not represented as independent Node 22 or emulator proof. [Run 33401110327](https://github.com/andrewbolaji/Chants/actions/runs/33401110327) independently supplies all eight clean-runner jobs at `5280c3a`, including Node 22 and all 23 transaction cases.

The approved follow-up changes only this record, spec, runbook, affected profile/roadmap status, execution evidence and CLI comments. It adds no executable control, test, resource, dependency or live authority.

| Observation | Disposition and proof boundary |
|---|---|
| Deferred cleanup is not monitored | Accepted; choose the review's explicit manual-gate option at E1/P5. Retained attempted rows differ from unfinished jobs, and a never-created row cannot be caught by a counter. Runbook names Andrew, evidence, private-canary cadence, escalation and the unresolved sustainable-public-observation gate. No automated coverage or event-loss reproduction is claimed |
| Submission cap serializes normal admission | Accepted as a capacity hypothesis, not measured public failure. Add isolated/concurrent and declared-launch-workload probes, explicit budgets and a recorded cap decision. Live probes and resource changes require later approvals |
| Deployer actAs omitted | Accepted; manifest, P1 and runbook name the exact runtime account, deployer permission, account-scoped binding and pre-deploy evidence. Actual grant/access remain unverified |
| Retry wording counts retries as attempts | Corrected to three total SDK attempts, at most two retries, matching installed Firestore types and maxAttempts: 3 |
| State-only success and replacement errors | Clarify inline and in the runbook: matching state proves neither authorship nor write capability; readback errors may replace the transaction cause. The algorithm remains unchanged |

Verification for this closure is the comment-free TypeScript syntax comparison, Node 22 build/unit suite, compiled endpoint inspection and staged governance in `docs/EXECUTION.md`, followed by the correction's exact-head PR receipt. No broad re-review or new test harness is introduced. A normal source revert removes only these clarifications; it cannot close any live HOLD.
