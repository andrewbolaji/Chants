...
This document explains the current Chants repository through the PR 34 exact green head `b952389e20b7c510f204f5cbbeb87af70b4c2574` plus the PR 35 store submission packet and its approved independent-review corrections. It includes inherited systems, creator and authentication, Living Songbook, the completed live catalog, operational preparation, policy and deletion, the Stage and Club Signal release presentation, and current store preparation. It is a reviewer map, not proof that the packet is merged, entered in either console, submitted, or released. Decisions 026 and 027 still own operational admission plus policy and deletion reasoning. Decision 030 owns the iPhone-only V1 and iPad V1.1 boundary. `docs/changes/2026-09-03-v1-store-submission-presentation-packet.md` owns the current bounded store work.
...
- **Known missing evidence:** final staged packet validation and exact-head CI for the corrected store block; merged release candidate; open-production walkthrough; live public-route and support readback; private reviewer access; exact distribution artifacts; final screenshot and current Google feature-graphic acceptance; Apple App Privacy and Google Data safety entry; questionnaire-derived store ratings; console metadata readback; submission; and release. Review credentials remain private and outside Git.
...
| Store submission presentation | Yes in current block | Canonical US English Apple and Google copy, iPhone-only V1 boundary, SDK-aware privacy worksheet, five-scene platform manifests, hash-bound icon and asset sources, release and screenshot evidence, and false-readiness guards; no console or store mutation |
...
`docs/STORE_PRIVACY_WORKSHEET.md` maps current source and included SDKs, not only user-entered fields. It covers account identity, public creator and chant content, comments and replies, optional 30-second video with recorded voice, interaction records, crash and diagnostic data, installation or attestation identifiers, IP metadata, and deletion. Crash linkage, IP-derived location treatment, Firebase processor status, and the linked but unconfigured Meta SDK remain exact-binary console checks. A hidden provider entry point does not remove a linked SDK from the privacy review.

`store/screenshots/manifest.json` defines five different product jobs for iPhone and Android: Stage, Clubs, chant reading, Create, and Saved Matchday Songbook. Each has exact source and output paths, SHA-256 evidence, and visible-truth requirements. `store/screenshots/frame.html` adds only a restrained headline, deck, sequence number, and Chants FC line around an exact release-candidate capture; a missing source stays visibly unpublishable. Decoded-pixel inspection rejects blank or placeholder-like files even when their size and format look correct. Final captures cannot include debug UI, another app, personal account data, maintenance or error states, fabricated engagement, club or player rights-sensitive assets, or a disabled feature. The old 1320 by 2663 development captures are intentionally rejected for the 1320 by 2868 iPhone set. V1 native targeting is iPhone-only; iPad layout and store evidence are pinned for V1.1.

`store/assets/google-feature-graphic.png` is a 1024 by 500 explicit-sRGB asset rendered by `scripts/render-google-feature-graphic.swift`. It uses one direct matchday promise and the established Chants supporter mark without unsupported affiliation, ratings, awards, or store badges. The renderer binds both app fonts and the supporter-mark input. The local HTML displays the canonical PNG instead of maintaining another composition. Andrew approved the exact canonical PNG on 7 September 2026. Separate approved asset and renderer digests match current evidence before the final gate can be true.

`scripts/check-store-submission.mjs` enforces field limits, identity, version, locale, the three-market boundary, the unchanged 17+ account rule, iPhone-only V1 native declarations, exact HTTPS trust routes, Git baseline ancestry, approved source drift, asset hashes, readiness-state coherence, scene inventory, release evidence, screenshot source and output hashes, decoded content, PNG size and alpha, Play icon safe-zone extent, and feature-graphic approval coherence. `scripts/test-store-submission.mjs` proves the valid prepared state and consequential known-bad mutations. The status cannot advance from `prepared_not_submitted` to `ready_for_submission` while any required merged-source, production, URL, support, reviewer, distribution, screenshot, feature-graphic, privacy, rating, or console gate is false.
...
| `flutter_facebook_auth` | Native Meta login and Firebase credential exchange | Hidden and unconfigured, but `FBSDKCoreKit` and `FBSDKLoginKit` remain linked and therefore stay inside exact-binary privacy review |
...
|---|---|
| Store packet validator and focused tests | PASS locally in honest `prepared_not_submitted` state: field limits, identity, locale, version, markets, 17+ rule, iPhone-only V1 targeting, URLs, baseline ancestry and drift, asset hashes, readiness, release evidence, five-scene manifests, screenshot source and output hashes, decoded placeholder rejection, dimensions and alpha, stale development captures, 1024 App Store source, derived 512 Google Play icon, explicit-sRGB 1024 by 500 feature graphic, digest-bound owner approval, canonical preview parity, and two-platform frame contract; 19 focused tests pass |
