...
`store/assets/google-feature-graphic.png` is a 1024 by 500 RGB asset rendered from a reproducible local HTML source. It uses the same Stage and Songbook visual language without unsupported affiliation, ratings, awards, or store badges. The `02 / SONGBOOK` correction changed the exact bytes and therefore reopened visual acceptance. The packet keeps that gate false until Andrew approves the current digest.
